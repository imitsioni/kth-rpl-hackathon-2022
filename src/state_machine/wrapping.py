...
class BaxterWrapping:
	...
	def __init__(self):
		...
		self.opto_right = rospy.Subscriber('/optoforce_fingertip_wrench',
		                                   WrenchStamped, self.force_callback_right)
		self.opto_left = rospy.Subscriber('/optoforce_fingertip_wrench2',
		                                  WrenchStamped, self.force_callback_left)
	...
	def force_callback_right(self, msg):
		...
		if not self.opto_calibrated_right:
			...
			print("Calibration: offset_y = " + str(self.opto_offsets_right[1]) +
			      ", offset_z =  " + str(self.opto_offsets_right[2]))

		if np.abs(force_y - self.opto_offsets_right[1]
		         ) > self.opto_thresholds_right[0] or np.abs(
		             force_z -
		             self.opto_offsets_right[2]) > self.opto_thresholds_right[1]:
	...
	def force_callback_left(self, msg):
		...
		if np.abs(force_y - self.opto_offsets_left[1]) > self.opto_thresholds_left[
		    0] or np.abs(force_z -
		                 self.opto_offsets_left[2]) > self.opto_thresholds_left[1]:
		...
		self.opto_forces_left = np.array([
		    ...
		    force_y - self.opto_offsets_left[1], force_z - self.opto_offsets_left[2]
		])
				...
				self.grasping_points[tag_ids.index(
				    tag.id[0])] = self.transform_to_world(
				        np.array([
				            ...
				            tag.pose.pose.pose.position.y, tag.pose.pose.pose.position.z
				        ]))
	...
	def cancel_action(self):
		...
		print("Removing last item from action list: prev_len = " + str(prev_len) +
		      ", new_len = " + str(len(self.list_actions)))
	...
	def place(self, A, B, l_edge):
		...
		directions, distances = self.alternative_waypoints(A, B, EE_pose, l_edge,
		                                                   0.06)
	...
	def alternative_waypoints(self, A, B, EE_pos, l_edge, h):
		...
		B = np.array([B.x, B.y, B.z])
		EE_pos = np.array([
		    EE_pos.pose.position.x, EE_pos.pose.position.y, EE_pos.pose.position.z
		])
	...
	def test_traj(self):
		...
		(plan,
		 fraction) = self.right_group.compute_cartesian_path(waypoint_list,
		                                                     0.001,
		                                                     0.0,
