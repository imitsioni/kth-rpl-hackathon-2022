...
class BaxterWrapping:
    ...
    def __init__(self):
        ...
        self.tfBuffer = tf2_ros.Buffer()
        self.listener = tf2_ros.TransformListener(self.tfBuffer)
        self.world_camera_transform = self.tfBuffer.lookup_transform(
            "world",  # source frame
            "camera_link",
            rospy.Time(0),
            # get the tf at first available time
            rospy.Duration(10.0))  # wait for 1 second
        self.camera_world_transform = self.tfBuffer.lookup_transform(
        ...
        self.right_gripper_world_transform = self.tfBuffer.lookup_transform(
            "right_gripper",  # source frame
            "world",
            rospy.Time(0),
            # get the tf at first available time
            rospy.Duration(10.0))  # wait for 1 second
        ...
        ##### SUBSCRIPTIONS
        self.left_ee = rospy.Subscriber('/robot/limb/left/endpoint_state',
                                        EndpointState, self.left_ee_callback)
        self.right_ee = rospy.Subscriber('/robot/limb/right/endpoint_state',
                                         EndpointState, self.right_ee_callback)
        self.opto_right = rospy.Subscriber('/optoforce_fingertip_wrench',
                                           WrenchStamped,
                                           self.force_callback_right)
        self.opto_left = rospy.Subscriber('/optoforce_fingertip_wrench2',
                                          WrenchStamped,
                                          self.force_callback_left)
        self.tags_detected = rospy.Subscriber("/tag_detections",
    ...
    def force_callback_right(self, msg):
        ...
        if not self.opto_calibrated_right:
            ...
            print("Calibration: offset_y = " +
                  str(self.opto_offsets_right[1]) + ", offset_z =  " +
                  str(self.opto_offsets_right[2]))

        if np.abs(force_y -
                  self.opto_offsets_right[1]) > self.opto_thresholds_right[
                      0] or np.abs(force_z - self.opto_offsets_right[2]
                                   ) > self.opto_thresholds_right[1]:
            self.interrupt_movement_right = True
            # print("STOP movement")
        else:
            self.interrupt_movement_right = False

        self.opto_forces_right = np.array([
            force_x - self.opto_offsets_right[0],
            force_y - self.opto_offsets_right[1],
            force_z - self.opto_offsets_right[2]
        ])
    ...
    def force_callback_left(self, msg):
        ...
        if not self.opto_calibrated_left:
            ...
            print("Calibration: offset_y = " + str(self.opto_offsets_left[1]) +
                  ", offset_z =  " + str(self.opto_offsets_left[2]))

        if np.abs(force_y -
                  self.opto_offsets_left[1]) > self.opto_thresholds_left[
                      0] or np.abs(force_z - self.opto_offsets_left[2]
                                   ) > self.opto_thresholds_left[1]:
            self.interrupt_movement_left = True
            # print("STOP movement")
        else:
            self.interrupt_movement_left = False

        self.opto_forces_left = np.array([
            force_x - self.opto_offsets_left[0],
            force_y - self.opto_offsets_left[1],
            force_z - self.opto_offsets_left[2]
        ])
    ...
    def update_grasping_points(self, msg):
        ...
        for tag in msg.detections:
            if tag.id[0] in tag_ids:
                self.grasping_points[tag_ids.index(
                    tag.id[0])] = self.transform_to_world(
                        np.array([
                            tag.pose.pose.pose.position.x,
                            tag.pose.pose.pose.position.y,
                            tag.pose.pose.pose.position.z
                        ]))
                if self.grasping_points[tag_ids.index(
                        tag.id[0])][2] > threshold:
    ...
    def calculate_release_orientation(self, movedir):
        ...
        world_gripper_trafo = self.tfBuffer.lookup_transform(
            "world",  # source frame
            link_name,
            rospy.Time(0),
            # get the tf at first available time
        ...
        quat_ = [
            rot_axis[0] * sin(alpha / 2), rot_axis[1] * sin(alpha / 2),
            rot_axis[2] * sin(alpha / 2),
            cos(alpha / 2)
        ]
    ...
    def cancel_action(self):
        ...
        print("Removing last item from action list: prev_len = " +
              str(prev_len) + ", new_len = " + str(len(self.list_actions)))
    ...
    def place(self, A, B, l_edge):
        ...
        directions, distances = self.alternative_waypoints(
            A, B, EE_pose, l_edge, 0.06)
        ...
        # Get EE position
        EE_pose = group.get_current_pose()
        EE_pose.pose.orientation = self.calculate_release_orientation(
            directions[0])
    ...
    def alternative_waypoints(self, A, B, EE_pos, l_edge, h):
        ...
        B = np.array([B.x, B.y, B.z])
        EE_pos = np.array([
            EE_pos.pose.position.x, EE_pos.pose.position.y,
    ...
    def move_along_line(self, direction, dist=0.01):
        ...
        for i in range(n_waypoints):
            ...
            new_pose.position.x += move_direction[0] * dist * (i +
                                                               1) / n_waypoints
            new_pose.position.y += move_direction[1] * dist * (i +
                                                               1) / n_waypoints
            new_pose.position.z += move_direction[2] * dist * (i +
                                                               1) / n_waypoints
    ...
    def ik_control(self, cartesian_point):
        ...
        joint_names = [
            k + "_e0", k + "_e1", k + "_s0", k + "_s1", k + "_w0", k + "_w1",
            k + "_w2"
        ]
        for name in joint_names:
            group.set_joint_value_target(
                name, ik_response.solution.joint_state.position[
                    ik_response.solution.joint_state.name.index(name)])
        group.go(wait=True)

    def rabp_go_base_pose(self):
        print("-- right arm base pose --")
        self.right_group.set_joint_value_target("right_e0",
                                                -0.2661456666981193)
        ...
        self.right_group.set_joint_value_target("right_s1",
                                                -0.9345777950191884)
        ...
        self.right_group.set_joint_value_target("right_w2",
                                                -0.5648884251388037)
