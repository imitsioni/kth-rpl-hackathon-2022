...
def get_pose_stamped_str(ps):
    return "["+str(ps.pose.position.x)+", "+str(ps.pose.position.y)+", "+str(ps.pose.position.z)+"]"
...
def plan_group(group, dest_pose, sleep_time, printed_str = ""): 
    ...
    print(printed_str+" ---- Goal:"+get_pose_stamped_str(dest_pose)+", From:" + get_pose_stamped_str(curr_pose))
